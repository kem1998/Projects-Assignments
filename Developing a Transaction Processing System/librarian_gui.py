...
from librarian_dao import LibrarianDAO # To communicate with Student table
...
class LibrarianGUI():
    ...
    def __init__(self):   
        ...
        # Messagebox title
        self.mb_title_bar = "Librarian CRUD"
    ...
    def create_gui(self, root):
        ...
        form_frame = tk.Frame(libr_frame)
        form_frame.pack()
        ...
        tk.Label(form_frame, font=('arial', 10), 
                 text = "Librarian").grid(row=0, column=0, columnspan=3)

        # row 1: student_id label, student_id entry and list_of_ids label
        tk.Label(form_frame, text= "Librarian Id", font=('arial', 10), width=20, 
        ...
        # Use the width= option to specify how wide in terms of number of characters
        tk.Entry(form_frame, textvariable=self.librarian_id, width=30, bd=1, 
                 state=tk.DISABLED).grid(row=1, column=1)
        # student_id is disabled to prevent user from entering a value
        # student_id is generated by the database because AUTOINCREMENT 
        # was specified in the database schema
        tk.Label(form_frame, text= "Librarian IDs", 
        ...
        tk.Entry(form_frame, textvariable=self.email_address, width=30, bd=1).grid(row=5, column=1)
        ...
        button_frame = tk.Frame(libr_frame, pady=10) 
        button_frame.pack()
    ...
    def save(self):
        ...
        If an student_id is present, the data is updated
        ...
        if valid_data:
            if (len(data['librarian_id'])==0):
                # If nothing has been entered in student_id 
                # i.e. its length is zero characters
                print("Calling create() as librarian_id is absent")
                self.create(data)
            else:
                print("Calling update() as librarian_id is present")
                self.update(data)
                pass
        else:
    ...
    def get_fields(self):
        ...
        Return:
            libr: dictionary object containing all the information 
                 about an student
        """

        libr = {}
        # student_id is ignored when creating a record
        libr['librarian_id'] = self.librarian_id.get() 
        libr['first_name'] = self.first_name.get()
        libr['last_name'] = self.last_name.get()
        libr['email_address'] = self.email_address.get()
        libr['mobile_number'] = self.mobile_number.get()
        return libr    
    ...
    def validate_fields(self, data):
        ...
        # Do not check student_id as this is generated by the database
        # Do not check date_of_birth 
        #if len(data['student_id']==0:
        #    valid_data = False
        #    message_list.append("student_id is empty")
    ...
    def create(self, data):
        ...
        print("Creating a Librarian ...")
        print(data)

        session = db.get_db_session() # Get a session (database.py)
        result = self.libr_dao.create(session, data) 
          # result is a tuple e.g. ("Student added successfully", 1004) 
        #result, student_id = self.stu.create(data) 
          # if you wish to get the message and student_id separately
    ...
    def update(self, data):
        ...
        print("Updating a librarian ...")
        print(data)

        session = db.get_db_session() # Get a session (database.py)
        result = self.libr_dao.update(session, data['librarian_id'], data)
        session.close() # close the session
    ...
    def delete(self):
        ...
        The student_id of the record to be deleted is obtained from a 
        global attribute.
        ...
        # Grab the student_id from the stringvar
        id = self.librarian_id.get() 
        print(id)
        
        # Call the data access object to do the job
        # Pass the id as parameter to the delete() method
        session = db.get_db_session() # Get a session (database.py)
        result = self.libr_dao.delete(session, id)
        session.close() # Close the session
    ...
    def load(self):
        ...
        session = db.get_db_session() # Get a session (database.py)
        result = self.libr_dao.find_ids(session) # {"student_ids": [1, 2, 3]}
        session.close() # Close the session
        print("result", result)
        # Check if there is an entry in the result dictionary
        if "librarian_ids" in result: 
            list_ids = result['librarian_ids'] # will crash if there is no entry!
            ...
            print("Setting librarian_id in listbox ...")
            for x in list_ids:
    ...
    def on_list_select(self, evt):
        ...
        value = w.get(index) 
          # value of the item clicked, in our case it's the student_id
        print(index) 
        print(value)

        # Call find_by_id and populate the stringvars of the form
        session = db.get_db_session() # Get a session (database.py)
        result = self.libr_dao.find_by_id(session, value)   
        session.close() # close the session
        print("result", result) 
           # { "student" : {"student_id": "", "firstname": "", etc}}
        libr = result['librarian']
        self.populate_fields(libr)
        pass

    def populate_fields(self, libr):
        """
        ...
        self.librarian_id.set(libr['librarian_id'])
        self.first_name.set(libr['first_name'])
        self.last_name.set(libr['last_name'])
        self.email_address.set(libr['email_address'])
        self.mobile_number.set(libr['mobile_phone'])
        pass
